    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
            <div class="lg:col-span-1">
                <img src="{{ $siteSettings->logoUrl() }}" alt="{{ $siteSettings->site_name }}" class="h-10 w-auto mb-4" width="140" height="40">
                @if($siteSettings->short_description)
                ...
                @if($siteSettings->socialLinks())
                    <div class="flex gap-3" aria-label="Social media links">
                        @foreach($siteSettings->socialLinks() as $platform => $url)
            ...
            <div>
                ...
                <address class="not-italic text-sm text-slate-400 space-y-2">
                    @if($siteSettings->email)
                        <p><a href="mailto:{{ $siteSettings->email }}" class="hover:text-xc-cyan transition-colors">{{ $siteSettings->email }}</a></p>
                    @endif
                    @if($siteSettings->phone)
                        <p><a href="tel:{{ preg_replace('/\s+/', '', $siteSettings->phone) }}" class="hover:text-xc-cyan transition-colors">{{ $siteSettings->phone }}</a></p>
                    @endif
