...
<article>
    <section class="xc-page-hero">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center scroll-reveal">
            <a href="{{ url('/blog') }}" class="text-sm text-xc-cyan hover:underline mb-4 inline-block">&larr; Back to Blog</a>
            @if($post->category)
    ...
    <section class="pb-20">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            @if($post->image)
                <img src="{{ asset('storage/' . $post->image) }}" alt="{{ $post->title }}" class="w-full rounded-xl mb-10 scroll-reveal">
            @endif
