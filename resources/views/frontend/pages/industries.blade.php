@php $pageKey = 'industries'; @endphp
@extends('frontend.layout.app')

@section('title', $industrySettings->meta_title ?: config('xcodrix.pages.industries.title'))
@section('meta_description', $industrySettings->meta_description ?: config('xcodrix.pages.industries.description'))
@section('canonical', config('xcodrix.domain') . '/industries')

@push('head')
@include('frontend.partials.schema-organization')
@endpush

@section('content')
<section class="xc-page-hero">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center scroll-reveal">
        @if($industrySettings->hero_badge)
            <span class="xc-badge mb-4 inline-block">{{ $industrySettings->hero_badge }}</span>
        @endif
        @if($industrySettings->hero_title)
            <h1 class="text-4xl md:text-5xl font-bold mb-4">{!! $industrySettings->hero_title !!}</h1>
        @endif
        @if($industrySettings->hero_subtitle)
            <p class="text-slate-400 text-lg max-w-2xl mx-auto">{{ $industrySettings->hero_subtitle }}</p>
        @endif
    </div>
</section>

<section class="py-20">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            @foreach($industries as $industry)
                <article class="xc-card scroll-reveal p-5">
                    <div class="w-12 h-12 rounded-xl bg-xc-cyan/10 flex items-center justify-center text-xc-cyan mb-4">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5"/></svg>
                    </div>
                    <h2 class="text-lg font-bold text-white mb-2">{{ $industry->name }}</h2>
                    @if($industry->description)
                        <p class="text-slate-400 text-sm leading-relaxed">{{ $industry->description }}</p>
                    @endif
                </article>
            @endforeach
        </div>

        @if($industrySettings->bottom_title || $industrySettings->bottom_content)
            <div class="max-w-3xl mx-auto mt-16 space-y-4 text-slate-400 leading-relaxed scroll-reveal">
                @if($industrySettings->bottom_title)
                    <h2 class="text-2xl font-bold text-white">{{ $industrySettings->bottom_title }}</h2>
                @endif
                @if($industrySettings->bottom_content)
                    <p>{{ $industrySettings->bottom_content }}</p>
                @endif
            </div>
        @endif
    </div>
</section>

@include('frontend.components.cta-banner')
@endsection
